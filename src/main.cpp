...
//add by kiyoshiro
template <typename Tpsys>
PS::F64 ReduceLipidPotentialEnergy(const Tpsys& psys){
  PS::F64 lipid_pot = 0.0;
  for(int i=0;i<psys.getNumberOfParticleLocal();i++) lipid_pot += psys[i].lipid_pot;
  return 0.5*PS::Comm::getSum(lipid_pot);
}
...
int main(int argc, char **argv){
  ...
  //  const PS::F64 time_offset = PS::GetWtime();//commented by kiyoshiro to check time after 100step
  PS::F64 time_offset;//add by kiyoshiro to check time after 100step
  PS::F64 time_calc_internal_force = 0.0;//add by kiyoshiro adviced by nomura-san
  for(int s=0; s<CONFIG.nstep; s++){
    PS::Comm::barrier();//add by kiyoshiro adviced by nomura-san
    if(s==100) time_offset = PS::GetWtime();//add by kiyoshiro to check time after 100step
#ifdef PARTICLE_SIMULATOR_REUSE_LIST
    ...
    if(s>100)//add by kiyoshiro to check time after 100step
