...
class TEA{
 private:
  ...
  static inline float log2f_tmp(float val) {
  ...
  template<int N>
  float gaussian_TEA(const bool pred,const unsigned int u,const unsigned int v ){
    ...
    float r = sqrtf( -2.0f * _LN_2 * log2f_tmp(v1 * _2_TO_MINUS_32) );
